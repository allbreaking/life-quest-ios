...
struct Routine: Codable, Identifiable, Equatable {
    ...
    func isDueToday() -> Bool {
        ...
        case .weekly:
            ...
            // Walk back up to 7 days to find the most recent scheduled occurrence.
            // If that occurrence hasn't been completed yet, carry the task forward.
            for offset in 0...6 {
        ...
        case .biweekly:
            ...
            // Walk back up to 14 days for the most recent biweekly occurrence.
            for offset in 0...13 {
                guard let checkDate = cal.date(byAdding: .day, value: -offset, to: todayStart) else { continue }
                let checkWeekday = cal.component(.weekday, from: checkDate)
                if days.contains(checkWeekday) {
        ...
        case .monthly:
            ...
            // Walk back up to 31 days to find the most recent scheduled date.
            for offset in 0...30 {
                guard let checkDate = cal.date(byAdding: .day, value: -offset, to: todayStart) else { continue }
